...
function DisplayCard({
  ...
  return (
    <div
      className={cn(
        "bg-muted/70 after:from-background hover:bg-muted relative flex h-52 w-[28rem] -skew-y-[8deg] select-none flex-col justify-between rounded-xl border-2 px-6 py-5 backdrop-blur-sm transition-all duration-700 ease-in-out after:absolute after:-right-1 after:top-[-5%] after:h-[110%] after:w-[20rem] after:bg-gradient-to-l after:to-transparent after:content-[''] hover:border-white/20 [&>*]:flex [&>*]:items-center [&>*]:gap-2",
        className
      )}
    >
      <div>
      ...
      <p className="text-wrap break-words text-lg leading-tight">
        {description}
      </p>
...
export default function DisplayCards({ cards }: DisplayCardsProps) {
  const defaultCards = [
    ...
    {
      className:
        "[grid-area:stack] translate-x-32 translate-y-40 hover:translate-y-20 transition-all duration-1000 ease-in-out"
    }
  ]
